...
func TestOfficialOptionalRegexFixtures(t *testing.T) {
	t.Parallel()
	...
	fixtures := []dialectFixture{
		{directory: "draft3", dialect: jsonschema.Draft3, files: []string{"non-bmp-regex.json"}},
		{
			directory: "draft4",
			dialect:   jsonschema.Draft4,
			files:     []string{"ecmascript-regex.json", "non-bmp-regex.json"},
		},
		{
			directory: "draft6",
